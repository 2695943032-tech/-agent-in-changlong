...
- `chimelong-pretrip-demo/`：完整可运行的 Nuxt 4 / Vue 3 全栈原型，覆盖游前规划、游中空间互动与游后回忆内容。
...
```bash
cd chimelong-pretrip-demo
pnpm install
pnpm dev
```

浏览器打开 `http://localhost:3000`。应用无需 AI Key 也可使用本地模板完成演示；如需接入模型，请复制 `.env.example` 为 `.env` 后在本地填写配置，切勿提交 `.env`。
