...
/* 管理台固定为纵向单列：每个业务模块完整占据一行，按页面向下浏览。 */
.admin-header,.overview-grid,.lower-grid,.stock-change-notice { max-width: 1040px; }
.overview-grid,.lower-grid { grid-template-columns: minmax(0, 1fr) !important; }
.stock-grid { grid-template-columns: minmax(0, 1fr); }
.report-list { max-height: none; overflow: visible; }
