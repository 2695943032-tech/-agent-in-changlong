...
<template>
  <section class="camera-stage" :style="{ '--ar-accent': props.accent }" aria-label="AR 实景画面">
    ...
    <div class="scan-frame" :class="{ scanning, detected }">
    ...
    <div v-if="detected" class="spatial-tag left-tag"><i>01</i><span>建议观赏距离</span><strong>8.4 m</strong></div>
    <div v-if="detected" class="spatial-tag right-tag"><i>02</i><span>当前状态</span><strong>悠闲进食中</strong></div>
...
.model-stage{position:absolute;z-index:1;top:20%;right:14%;bottom:23%;left:14%;pointer-events:none}.model-reveal-enter-active,.model-reveal-leave-active{transition:opacity .42s ease,transform .55s var(--ease-out)}.model-reveal-enter-from,.model-reveal-leave-to{opacity:0;transform:translateY(30px) scale(.78)}
