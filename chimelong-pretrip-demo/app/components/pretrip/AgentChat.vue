  ...
  <section class="chat-shell" data-release="2026-08-07-collaboration-release-2" :style="{ '--agent-accent': currentCompanion.accent }">
    ...
    <div v-else-if="plan && route.query.tab !== 'map' && !mapOpen" class="wechat-dock">
      ...
      <div v-if="toolsOpen" class="tool-grid">
        <button class="ar-tool" type="button" @click="navigateTo('/inpark/ar')"><i>AR</i><span>实景奇遇</span></button>
...
.animal-status { color: #42805a !important; font-weight: 800; }.composer-row { display: grid; grid-template-columns: minmax(0, 1fr) 34px 34px; align-items: center; gap: 8px; }.composer-row input { min-width: 0; height: 37px; padding: 0 11px; border: 0; border-radius: 7px; background: #fff; color: var(--ink); font-size: 13px; }.composer-row button { display: grid; width: 34px; height: 34px; place-items: center; border: 0; background: transparent; color: #31443d; font-size: 23px; line-height: 1; }.composer-row button:not(.plus-button) { transform: translateX(-10px); }.composer-row button.active { color: #ba6332; }.plus-button { font-size: 26px !important; }.photo-input { display: none; }.tool-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); padding: 15px 0 5px; gap: 0; }.tool-grid button { display: grid; justify-items: center; gap: 6px; border: 0; background: transparent; color: var(--ink); font-size: 11px; }.tool-grid i { display: grid; width: 48px; height: 48px; place-items: center; border-radius: 12px; background: #e3e5df; color: var(--forest); font-size: 25px; font-style: normal; }.tool-grid .ar-tool i { background: var(--forest); color: #fff; font-size: 19px; font-weight: 900; }.tool-grid span { position: relative; }.tool-grid span b { position: absolute; top: -56px; right: -13px; display: grid; min-width: 17px; height: 17px; padding: 0 4px; place-items: center; border-radius: 999px; background: #bd4c2e; color: #fff; font-size: 9px; }
